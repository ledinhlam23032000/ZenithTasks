...
Một Dự án mới có ngay khung module V2 gồm Tổ chức & vị trí, Cơ chế, Mô phỏng và Task. Admin có thể bật/tắt các module đã triển khai trong phần cấu hình của workspace; thao tác này chỉ thay đổi `enabledFeatures` và ghi audit, không xóa dữ liệu. Task đã có schema `ZWorkspaceTask`, route, create/list/status update, membership scope và isolation test. Khách hàng, Lịch hẹn, Tài chính và Lương/hoa hồng vẫn là module định hướng, chưa được bật cho đến khi có schema, route, quyền, migration và test độc lập.
...
AI hiện nhận workspace context rõ ràng gồm `workspaceKind` (INTERNAL hoặc PROJECT), `projectId` nếu là Dự án, capability, user scope và mục đích. Conversation/approval/audit có metadata scope; planner không nạp snapshot Nội Bộ khi ở Project và policy chặn tool thiếu/sai projectId. Các tool nghiệp vụ clinic-global chưa được giả vờ chuyển thành tool project-local; Admin toàn cục không đồng nghĩa với việc bỏ qua cảnh báo hoặc approval L5.
...
Các commit mới nhất trên `master` gồm `1710820` (Task local), `41aa7fc` (quản lý membership), `46642f7` (AI workspace scope), `fc8e244` (sidebar mở Trợ lý AI giữ project context) và `d1e5ada` (tài liệu/checkpoint). Dữ liệu clinic legacy vẫn là **Nội Bộ** và không bị di chuyển. `ZWorkspaceTask` dùng `projectId` bắt buộc; create/list/status update, membership active và isolation rollback test đã được triển khai. Customer, Appointment, Finance và Payroll project-local vẫn chưa được giao.
...
Trong clone sandbox, `prisma validate`, TypeScript, Next production build và 10 unit tests governance đã đạt. Máy clinic đã chạy đúng `Sua-Loi.bat` đến `DA XONG` cho master `d1e5ada`; checkout local và `origin/master` cùng SHA, app container mới chạy, log có `58 migrations found`, migration AI scope apply thành công, `Ready`, `/login` HTTP 200. Smoke `/du-an`, `/tro-ly` và các URL project không hợp lệ trả redirect auth 307 thay vì 500; log startup không có runtime error mới. Untracked `checks/test-task-isolation.sql` và `worktrees/` được giữ nguyên, không đưa vào stash/commit. UI authenticated Admin/Manager chưa được browser-verified trong phiên này nên không tuyên bố đã hoàn tất kiểm thử người dùng đăng nhập.
