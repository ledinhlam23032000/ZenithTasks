...
| P05-T04 | Xây payroll run local | P03-T06,P05-T03 | Chốt snapshot kỳ lương và tổng hợp commission theo project; sensitive fields có policy; hiện có CALCULATE → PREVIEW → APPROVE → two-person FINALIZE/VOID, chưa payout/accounting integration và runtime QA | review |
| P05-T05 | Xây finance/payroll role actions | P02-T02,P05-T01,P05-T04 | ADMIN/Project Manager/capability được phân tách; audit thay đổi nhạy cảm; hiện có Admin-only governance và masking số tiền non-Admin | review |
| P05-T06 | Viết test finance/payroll/mechanism không rò Nội Bộ | P05-T02,P05-T03,P05-T04 | Cross-project, wrong role, wrong mechanism và wrong period đều bị chặn hoặc trả đúng scope; hiện có pure calculation/policy/rule tests và rollback-only SQL, còn DB/role runtime | review |
...
| Mã | Task | Phụ thuộc | Đầu ra và tiêu chí nghiệm thu | Trạng thái |
|---|---|---|---|---|
| P08-T01 | Thêm AI GLOBAL context và global overview tool | P02-T05,P07-T04 | Admin chọn GLOBAL và overview aggregate scoped; hiện có selector `__GLOBAL__`, conversation scope và capped overview foundation, còn runtime | review |
| P08-T02 | Cho AI Admin chọn một hoặc nhiều project mục tiêu | P08-T01,P02-T04 | Target project explicit, kiểm tra tồn tại/quyền; hiện có selector project và server governance yêu cầu projectId ở Global, còn multi-target runtime | review |
...
| P08-T06 | Test AI INTERNAL/PROJECT/GLOBAL và prompt isolation | P08-T01,P08-T02,P08-T04 | AI không đọc chéo; GLOBAL chỉ Admin; preview/audit chứa đúng target; hiện có governance regression, còn integration/runtime prompt isolation | review |
