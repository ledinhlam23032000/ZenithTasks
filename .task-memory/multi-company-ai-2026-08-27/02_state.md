...
- **Updated:** 2026-08-28 13:10 GMT+7
...
- **Current phase:** MC-11.1 corrective → MC-11.2 persistent job storage; MC-13 client write-denial remains a separate blocked evidence lane.
...
PR #86 đã thêm QA-only client write-denial harness, có guard `QA_CONFIRM=YES`, database URL mang dấu hiệu QA/test/staging và quyền ADMIN. PR #87 đã thêm Global AI observability dashboard read-only cho Admin, hiển thị counts, child/global scope, heartbeat health, allowlist và audit lifecycle tối thiểu; không hiển thị prompt/raw tenant data và không có mutation control. Trên branch dedicated sau PR #89, MC-11.1 đã được corrective: PROJECT source bắt buộc source projectId và CHILD target cùng project; GLOBAL có thể target GLOBAL aggregate hoặc CHILD explicit với target project, không có implied tenant/raw access.
...
Code quality của credential wave và Global AI observability đã pass TypeScript, syntax check và Vitest 90 files/447 tests; PR #85, #86, #87 đều có CI push/pull_request xanh và đã merge. Corrective MC-11.1 + persistent MC-11.2 code-level gate hiện pass Prisma validate/generate, TSC và focused Vitest 1 file/6 tests; full Vitest/Next build/CI chưa chạy cho branch này. Build QA trước đó nhận diện route `/qa/write-denial` và route `/he-thong/ai-tong` trên source tương ứng.
...
Company mới vẫn DRAFT, chỉ ACTIVE nhận operational writes; ARCHIVED giữ dữ liệu/audit và chỉ archive/restore, không hard-delete. AI con chỉ được project-local read allowlist; AI Tổng chỉ aggregate/health/audit summary và không vượt RBAC, approval, audit hoặc isolation. ZAiJob persistence hiện chỉ queue metadata/empty arguments, lưu target/trace/policy/audit; chưa có worker, dispatcher execution, child write, usage persistence hay QA migration. Không triển khai migration, backup/rollback, payout thật, clinic updater hay production deploy trong wave này.
...
MC-04/MC-05 vẫn PARTIAL về completeness list/detail/aggregate ngoài các case đã có. MC-07 còn thiếu mutation adapter qua preview → approval → audit. MC-09 còn thiếu child heartbeat/usage đầy đủ. MC-10 nay có observability dashboard nhưng vẫn thiếu end-to-end dispatcher proof và usage model. MC-11 đã có corrected contract, additive ZAiJob schema/migration draft và guarded enqueue action ở code-level; vẫn thiếu full gates/CI, QA migration, worker/dispatcher, runtime reauthorization E2E, usage/heartbeat và child control. Payroll/accounting local, canonical workbook, clinic migration/deploy và owner approvals vẫn blocked/not started.

## Next 3 actions

1. Chạy full Vitest và Next build cho corrective/persistence branch; kiểm tra diff/migration SQL, rồi cập nhật CI qua PR nếu gates xanh.
2. Chỉ sau review và safety gate, apply migration trên `zenithqa` độc lập, chạy guarded seed → verifier → auth/export; không chạy clinic. Đồng thời khi My Browser reconnect, submit bốn form `/qa/write-denial` và giữ MC-13 PARTIAL/BLOCKED nếu thiếu evidence.
3. Sau mỗi wave cập nhật `07_task_ledger.md`, `06_changelog.md`, checkpoint này; chỉ mở dispatcher/worker hoặc owner gate cho migration/backup/clinic khi có explicit approval và rollback evidence.
