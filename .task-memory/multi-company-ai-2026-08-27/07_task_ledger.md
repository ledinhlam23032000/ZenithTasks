...
| MC-17 | `generate_commission_draft` dùng engine hoa hồng THẬT (`lib/commission-data.ts`) thay vì `amount*rate` tự do do caller đưa vào | MC-07 | TODO | — | Chủ 2026-08-29 phê bình đúng: tool này "đẹp số liệu chưng cho đẹp", không đọc dữ liệu bán hàng thật. Owner yêu cầu làm lần lượt 1->7, đây là mục 1 | Đọc kỹ `commission-data.ts` trước khi sửa — công thức đã có test, không viết lại logic, chỉ nối V2 job engine vào đúng hàm đã có |
