...
> agent song song (tài chính/kế toán, vận hành/hồ sơ, khách hàng công khai/quản trị) tìm được
> 15 lỗi; lỗi gốc (double-count Nợ KH còn lại + DS chốt do cộng trùng khi 1 người vừa là tư
> vấn viên vừa bác sĩ trên cùng hồ sơ) đã được xử lý riêng (xem commit "dismiss warnings and
> deduplicate staff debt" + "add safe revenue allocation for staff"). Đợt này sửa 13 lỗi còn lại.
- **Dịch vụ tặng/miễn phí bị tính lại theo giá gốc**: `financial-summary.ts` thêm `normalizeServicePrice()` — khi nhân viên để "Giá ưu đãi"=0 để tặng (còn giá gốc>0), `addCaseService`/`updateCaseService` (`ho-so/actions.ts`) giờ lưu dưới dạng giảm giá 100% thay vì giữ `unitPrice=0` (vốn trùng hình dạng dữ liệu hồ sơ cũ thiếu giá mà `lineBaseFor()` tự động bù bằng giá gốc).
- **Xóa hồ sơ/khách hàng không hoàn kho vật tư đã trừ**: thêm `restoreMaterialUsageStock()` dùng chung, gọi trước khi `deleteCase`/`deleteCustomer` xóa `MaterialUsage` — trước đây tồn kho bị trừ vĩnh viễn dù ca điều trị không còn tồn tại.
- **Trang Khách hàng: tab lọc "Đã làm/Chưa làm dịch vụ" không khớp cột Trạng thái**: với vai trò Tư vấn viên/Bác sĩ, bộ lọc tab (`khach-hang/page.tsx`) giờ dùng chung `caseScope` với cột Trạng thái mỗi dòng — trước đây tab tính theo TOÀN BỘ ca của khách (kể cả ca đồng nghiệp) trong khi cột Trạng thái chỉ tính ca của mình.
- **Xóa nhân sự (xóa cứng) làm mất vĩnh viễn Nhật ký kiểm toán**: `deleteStaff` (`nhan-su/actions.ts`) giờ chặn xóa cứng nếu nhân sự đã có lịch sử (AuditLog/CaseRecord/CaseService/Payment/Attendance/PayrollEntry) — `Attendance`/`PayrollEntry` bị XÓA VĨNH VIỄN theo `onDelete: Cascade`, còn lại bị `SET NULL` mất tên actor. Hướng dẫn dùng "Ngừng hoạt động" thay thế. `DeleteButton` cũng sửa để hiện đúng lý do lỗi thay vì thông báo chung chung.
- **Chống trùng lịch hẹn bỏ sót Lịch tái khám**: `consultantConflictMessage` (`lich-hen/actions.ts`) giờ xét cả bảng `FollowUp`, trước đây chỉ xét `Appointment` nên có thể đặt lịch hẹn mới trùng giờ bác sĩ đã có hẹn tái khám mà không cảnh báo.
- **Trùng tin nhắn webhook Zalo/Facebook (race condition)**: `Message.externalId` đổi từ index sang ràng buộc UNIQUE (migration `message_external_id_unique`); `recordInboundMessage()` (`channels/conversations.ts`) gộp tạo hội thoại + tin nhắn vào 1 transaction, bắt lỗi P2002 thay vì kiểm tra-rồi-ghi tách rời 2 bước.
- **Đua refresh token Zalo khi 2 người gửi tin cùng lúc**: `ensureZaloAccessToken` (`channels/zalo.ts`) thêm khoá trong tiến trình theo `channelAccountId` — Zalo xoay refresh_token mỗi lần dùng nên 2 request đồng thời dùng chung 1 token cũ trước đây khiến request tới sau luôn bị từ chối.
- **Nhật ký hệ thống thiếu nhãn cho phần lớn hành động**: `status.ts` thêm `AUDIT_ACTION_LABEL` làm nguồn nhãn+tone DUY NHẤT (trước đây `nhat-ky/page.tsx` và `export/route.ts` mỗi nơi tự định nghĩa 1 bản riêng, chỉ 15/90+ loại — thiếu cả `EDIT_CASE_DATE`, `EDIT_ATTENDANCE`, `CLOSE_PERIOD`, `PAY_SALARY`...).
- **Widget "Hoạt động nhạy cảm gần đây" ở Tình trạng hệ thống không lọc theo nhạy cảm**: `system-status.ts` thêm `isSensitiveAuditAction()` (tự nhận `DELETE_*`/`REVEAL_*` + danh sách bổ sung) lọc đúng, trước đây chỉ lấy 10 dòng nhật ký mới nhất bất kể loại.
- **Quyền xuất file trang Lương lệch với trang chính**: `luong/export` + `luong/export-ke-toan` đổi từ `requireUser(["ADMIN","MANAGER"])` sang `requireCap("mod:luong")` khớp trang chính.
- **Sổ công nợ bỏ sót phạm vi vai trò Bác sĩ**: `cong-no/page.tsx` + `export/route.ts` bổ sung scope `doctorId` cho vai trò DOCTOR, trước đây chỉ lọc CONSULTANT.
- **Hẹn nợ tính thừa 1 kỳ**: `duePeriods()` (`debt-plan.ts`) — khi ngày hẹn trả hằng tháng sớm hơn ngày bắt đầu kế hoạch trong cùng tháng, kỳ đầu giờ dời đúng sang tháng sau thay vì tính nhầm kỳ của chính tháng đó (đã trôi qua trước khi kế hoạch tồn tại) là "đã tới hạn".
- **Dọn `case-math.ts` chết**: file không còn được gọi ở production (đã chuyển hẳn sang `financial-summary.ts` từ trước) — xóa file + test, sửa lại các chỗ tài liệu/comment còn trỏ nhầm.
- **Kiểm thử**: TSC sạch, ESLint sạch (không tính 3 cảnh báo/lỗi có sẵn từ nhánh khác không liên quan), 292/292 test (thêm test cho `sumByResponsibleStaff`-tương đương, `normalizeServicePrice`, `duePeriods` kịch bản ngày hẹn sớm hơn ngày bắt đầu). Chạy thử qua server thật (cookie phiên forge cho tài khoản ADMIN seed, theo đúng quy ước mục 10) xác nhận `/luong`, `/cong-no`, `/khach-hang`, `/nhat-ky`, `/he-thong`, `/nhan-su`, `/lich-hen` cùng các route export đều tải đúng (HTTP 200, không lỗi runtime), dropdown "Hành động" ở Nhật ký hiện đủ ~91 lựa chọn.
- **Lưu ý cho phiên sau**: trong lúc sửa, phát hiện có tác vụ tự động (`Tu-Dong-Cap-Nhat.ps1`/tương tự) liên tục `git reset --hard origin/master` trên máy này trong khi có phiên khác đang push đồng thời — làm mất các sửa đổi CHƯA COMMIT nhiều lần giữa chừng. Đã khắc phục bằng cách commit + push thành nhiều đợt nhỏ ngay sau mỗi nhóm sửa. Nếu gặp lại tình trạng "vừa sửa xong đã biến mất", kiểm tra `git reflog` trước khi nghi ngờ do mình thao tác sai.
...
### Task 10 hộp thư đa kênh — giao diện xử lý chung và nhật ký cũ (2026-08-01)
- Khi `OMNICHANNEL_ENABLED=true` và người dùng có `inbox.view`, `/cham-soc` mở hộp thư mới với ba hàng chờ Chưa phân công/Của tôi/Tất cả, lọc Zalo OA/Facebook, badge trạng thái kênh, số chưa đọc và cảnh báo quá mục tiêu phản hồi. Tắt cờ hoặc không có quyền thì trang tiếp tục hiển thị đúng nhật ký thủ công cũ.
- Khung hội thoại cho phép nhận/phân công, mở–tạm hoãn–đóng, trả lời text, gửi JPEG/PNG/WebP/PDF, thử lại cùng tin thất bại, ghi chú nội bộ, xem đồng nghiệp đang xem/soạn và liên kết hồ sơ khách đã xác minh. Lỗi provider được chuyển thành nhãn sạch; view model không serial hóa lỗi nội bộ hoặc token.
- Poller làm mới 5 giây chỉ khi tab đang hiện, refresh ngay khi focus/quay lại tab và gửi heartbeat không chặn giao diện. Desktop dùng ba cột; mobile đi theo luồng danh sách → hội thoại → ngữ cảnh khách, có nút quay lại và không tràn ngang.
- TDD view model RED khi module chưa tồn tại, sau triển khai GREEN **5/5**; tổng suite tăng từ 68 lên **73 test**. Playwright trên PostgreSQL 16 tạm đạt ở 1440×1000 và 390×844, `scrollWidth=clientWidth=390`, console không có lỗi; tab thủ công vẫn hiển thị đủ **19** dòng seed cũ. Container production và dữ liệu vận hành không bị restart/chỉnh sửa.

### Task 11 hộp thư đa kênh — cấu hình, vận hành và cổng phát hành (2026-08-01)
- Đã bổ sung đầy đủ biến môi trường cho Zalo OA/Meta, cờ `OMNICHANNEL_ENABLED=false` mặc định, URL công khai, callback/webhook, khóa mã hóa token và secret bảo trì. Entrypoint tự sinh khóa kênh 32 byte cùng secret bảo trì một lần trong volume `.runtime`; thư mục dùng mode `0700`, tệp khóa dùng `0600`. Volume tệp inbox riêng ở ngoài `public/` cũng dùng `0700`.
- Bộ cài Windows `Cai-Bao-Tri-Kenh.ps1/.bat` đăng ký tác vụ POST `/api/internal/channels/maintenance` mỗi 12 giờ. TDD đã quan sát RED khi script chưa tồn tại rồi GREEN; test xác nhận đúng URL, chu kỳ, nguồn đọc secret và không in secret. `Sao-Luu.ps1` nay sao lưu cả `.runtime` và tệp inbox; bản sao chứa khóa phải được hạn chế quyền truy cập và mã hóa ở nơi lưu.
- Hướng dẫn vận hành đã ghi rõ callback, quyền cần xin duyệt, test user, kết nối lại, health check, backup/restore, rollback bằng feature flag và nguyên tắc không đưa App Secret/token vào Git hoặc chat. Toàn bộ 9 lỗi ESLint cũ còn lại cũng được sửa đúng nguyên nhân; lint toàn repository hiện sạch lỗi.
- Cổng phát hành chạy từ bản xuất Git sạch ngoài OneDrive: `prisma format --check`, `prisma validate`, `prisma generate`, Vitest **73/73**, `tsc --noEmit`, ESLint toàn repo và Next.js production build đều đạt. Dockerfile chuẩn bị chặn ở `npm ci` do registry npm không phản hồi trong hơn 10 phút; để kiểm chứng phần source/build mà không giả nhận mạng đã đạt, image phát hành được dựng thành công trên lớp dependency khóa sẵn của `zenithtasks-app:latest`, sau đó generate Prisma và build Next đầy đủ.
- Runtime image tạm đạt: `/login` trả 200; bearer sai của maintenance trả 401, bearer đúng trả 200; quyền runtime/độ dài khóa đúng; khóa không xuất hiện trong Git, HTML hoặc log. Diễn tập PostgreSQL 16 áp 18 migration, seed rồi áp migration thứ 19 giữ nguyên `CareMessage` **16 trước / 16 sau** và tạo bảng `ChannelAccount`. Fixture tích hợp gửi trùng cùng webhook chỉ tạo `messages=1 conversations=1 receipts=1` rồi tự dọn.
- Toàn bộ container/network/volume kiểm thử đã xóa; container production `zenithtasks-app-1` vẫn chạy và không bị restart. Mã nguồn hoàn tất **11/11 task**. Checkpoint bên ngoài vẫn chủ động để mở: chưa bật cờ hay kết nối tài khoản thật cho đến khi ADMIN nhập App ID/secret trực tiếp trên máy vận hành, provider phê duyệt quyền và hoàn tất một vòng OAuth + nhận/trả lời bằng test user thật.
...
- Ba URL pháp lý đã được deploy và lưu bền trong Meta.
- Cổng build Next.js 16 được sửa đúng quy ước Route Handler: `verifyMetaWebhook`/`handleMetaWebhook`, `handleZaloWebhook` và `handleMaintenanceRequest` chuyển ra `lib/channels`; các tệp `route.ts` chỉ còn export HTTP method/config hợp lệ. Test ingestion/maintenance giữ nguyên hành vi và production build đã qua TypeScript/prerender.
- Khi thêm `pages_messaging` vào App Review, Meta yêu cầu xác định ứng dụng là **Tech Provider**; màn hình cảnh báo lựa chọn này không thể hoàn tác và kéo theo Xác minh doanh nghiệp, Xác minh quyền truy cập cùng bộ câu hỏi Xét duyệt ứng dụng. Chưa bấm `Continue` khi chưa có xác nhận riêng của chủ dự án. Trong thời gian chờ, kết nối thật dùng được cho tài khoản có vai trò app/test; khách đại trà chỉ hoạt động sau khi Meta phê duyệt quyền và app được phát hành.
...
### Gộp nhánh, dừng App Review Meta (2026-08-17)
- Song song với nhánh trên, nhánh `claude/lucid-cori-fg136w` đã tự xây một bản hộp thư Zalo/Facebook khác — schema và code hoàn toàn riêng, không dùng chung gì với thiết kế trên — và đây mới là bản THẬT SỰ đang chạy trên máy chủ (đã kiểm chứng trực tiếp trên database).
- Chủ dự án xác nhận **không tiếp tục** hồ sơ App Review Meta (`1355889016681500` / hồ sơ doanh nghiệp `873415778937392`, dừng ở trạng thái "Đang xem xét" từ 2026-08-01). Toàn bộ code domain (`lib/channels/{provider,connect,maintenance,token-manager,*-webhook,ingest,inbox,crypto,attachments,signatures}.ts`, route `api/channels/**`, UI `cham-soc/{inbox-shell,inbox-*,conversation-*,customer-context,inbox-poller,manual-log}`, model `ChannelAccount`/`ChannelContact`/`ChannelThread`/`Conversation`/`ConversationEvent`/`ConversationPresence`/`InboxMessage`/`InboxAttachment`/`OAuthAttempt`/`WebhookReceipt` kiểu cũ) đã bị xoá khi gộp nhánh về `master`. Giữ lại 3 trang pháp lý (`/chinh-sach-quyen-rieng`, `/dieu-khoan-su-dung`, `/xoa-du-lieu`) vì không phụ thuộc code đã xoá.
- Nếu sau này muốn làm lại hộp thư Facebook cho thật: đọc lại phần thiết kế phía trên để tham khảo (đặc biệt phần RBAC/bảo mật/khảo sát Pancake-respond.io-SleekFlow-Intercom-Zendesk), nhưng viết lại trên nền model `ChannelAccount`/`Conversation`/`Message` đơn giản đang chạy — KHÔNG khôi phục nguyên code cũ vì chưa từng qua App Review, không rõ còn khớp chính sách Meta hiện hành hay không.
