...
- **assistant.ts** — trợ lý AI hỏi-đáp THUẦN (có test, D1): `formatAssistantContext` (ảnh chụp số liệu → văn bản gọn), `BUSINESS_RULES_KNOWLEDGE` (giải thích cơ chế nghiệp vụ THẬT — giá dịch vụ, lương, công nợ, hạng thành viên, Lãi/Lỗ — viết tay bằng ngôn ngữ kinh doanh, KHÔNG thuật ngữ code; sửa công thức ở lib nào thì cập nhật đoạn tương ứng ở đây, đối chiếu mục 7 bên dưới), `ASSISTANT_SYSTEM`, `SUGGESTED_QUESTIONS`. **assistant-data.ts**: `getAssistantContext()` gom số liệu KINH DOANH (KHÔNG gồm SĐT/y khoa) — tái dùng `getBusinessAnalytics`. Dùng ở `/tro-ly` (module `tro-ly`, icon `Sparkles`, **CHỈ ADMIN + SHAREHOLDER** — ranh giới cứng ở `userCan()`, không có MANAGER dù trang gần giống `/ke-hoach`); action `askAssistant` đưa CẢ bối cảnh số liệu LẪN `BUSINESS_RULES_KNOWLEDGE` cho `ai.ts` — AI KHÔNG truy cập thẳng DB (chống bịa số/rò rỉ). 🔑 cần `AI_API_KEY`.
  - **assistant-request-ai.ts** — trợ lý KHÔNG được tự sửa hệ thống (rủi ro quá lớn trên app tính tiền/lương thật). Thay vào đó: nút "Ghi thành yêu cầu cho lập trình" dưới mỗi câu trả lời (`tro-ly/assistant-chat.tsx`) gọi action `logAssistantChangeRequest` — AI tóm tắt đoạn hỏi-đáp thành `{title, note}` (JSON, cùng khuôn AI-JSON ở mục 8.10/`plan-ai.ts`; AI tắt hoặc trả sai định dạng thì fallback dùng nguyên văn câu hỏi, KHÔNG chặn việc ghi nhận), rồi tạo 1 `PlanTask` dưới 1 `Plan` dùng chung tên cố định **"Yêu cầu từ Trợ lý AI"** (tự tìm-hoặc-tạo, `aiGenerated: true`) — admin/cổ đông xem lại ở `/ke-hoach` như 1 hộp thư việc cần làm, tự chọn lọc rồi giao cho lập trình (vd 1 phiên Claude Code) xử lý có kiểm tra, KHÔNG có gì tự động áp dụng vào code/schema.
