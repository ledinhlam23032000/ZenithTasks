...
>
> **Phiên bản hiện tại:** trước khi làm việc, đọc [`../VERSION.md`](../VERSION.md), [`../CHANGELOG.md`](../CHANGELOG.md) và [`../docs/INDEX.md`](../docs/INDEX.md). Sau đó đọc [`../docs/AI-ADMIN-GATEWAY.md`](../docs/AI-ADMIN-GATEWAY.md) nếu chạm vào AI, quyền, tiền, hồ sơ hoặc approval; đọc [`../docs/OPERATIONS-RUNBOOK.md`](../docs/OPERATIONS-RUNBOOK.md) nếu chạm vào production. Commit chuẩn hiện tại, các migration gần đây, Agent AI và workflow hộp thư được mô tả ở đó. Không dùng các mô tả cũ bên dưới nếu `VERSION.md` đã cập nhật khác.
...
- **commission.ts** — TÍNH HOA HỒNG NHÂN SỰ theo đúng "Cơ chế lương nhân sự Trung Tâm" (văn bản chủ cung cấp) — toán THUẦN, có test: `lunchAllowance` (25k/ngày công), `doctorServiceCommission` (8% dịch vụ bác sĩ trực tiếp làm), `doctorConsultCommission` (10% tư vấn khách CŨ — khách mới 0%), `nurseServiceFee` (100k/ca dịch vụ điều dưỡng phụ trách), `nurseConsultCommission` (4% tư vấn), `consultantCommission` (bậc TRỌN GÓI theo doanh số tích luỹ trong tháng, tách khách mới/khách cũ — `CONSULTANT_NEW_BRACKETS`/`CONSULTANT_RETURNING_BRACKETS`), `computeCommissionBreakdown` (gộp tất cả thành 1 breakdown, nhận `attendanceWage` làm THAM SỐ chứ KHÔNG tự tính lại — dùng lại `payroll-pure.ts` `computeBaseActual()` để tránh 2 công thức lương-theo-công lệch nhau). "Khách CŨ" = đã từng làm ca VỚI ĐÚNG nhân sự đó trước đây (không phải khách cũ nói chung).
- **commission-data.ts** — lớp thu thập dữ liệu (DB) cho `commission.ts`: `getCommissionForMonth(monthDate, standardDays)` → `Map<userId, {role, breakdown, details}>` cho MỌI nhân sự đang hoạt động trong tháng — tự phân loại khách mới/cũ theo lịch sử hồ sơ SERVICED/COMPLETED trước đó, gom theo vai trò (bác sĩ/điều dưỡng/tư vấn viên — vai trò khác không có công thức, bỏ qua). `details: CommissionCaseDetail[]` = từng dòng đóng góp hoa hồng (ngày, hồ sơ, khách hàng, loại, tỉ lệ, giá trị, số tiền) — dùng cho phụ lục hoa hồng. CHỈ ĐỌC, không ghi DB — `/luong` hiển thị làm GỢI Ý (nút "Dùng số này" trong `PayrollEditButton`), `PayrollEntry.commission` vẫn là số ĐÃ LƯU, không tự ghi đè. GIẢ ĐỊNH chưa xử lý được (thiếu căn cứ rõ trong văn bản, xem comment đầu file): "khách có cọc lại tiền" và "khách bảo hành" (loại trừ khỏi khách cũ tư vấn viên) — app chưa có khái niệm này; bậc hoa hồng tư vấn viên tính trên GIÁ TRỊ CHỐT (không phải thực thu) — đổi được ở đây nếu chủ muốn.
- **commission-appendix.ts** — `buildCommissionAppendixSheet(staffName, monthDate, details)` → 1 `RichSheet` liệt kê từng dòng hoa hồng của 1 nhân sự (đúng bố cục mẫu chủ gửi) — trả `null` nếu không có dòng nào (không tạo sheet rỗng).
- **payroll-letterhead.ts** — `buildPayrollLetterheadSheet(monthDate, standardDays, rows)` → `RichSheet` đúng mẫu công ty (tiêu đề + quốc hiệu + lưới ngày công 1..N + Công thực tế/Phụ cấp cơm trưa/Hoa Hồng Tư Vấn/Tiền Thủ Thuật/Thưởng-ĐC/Tổng cộng hưởng lương + chữ ký 3 bên). "Tổng cộng hưởng lương" ở đây GỒM phụ cấp cơm trưa (đúng công thức thật trong văn bản chủ gửi) nên KHÁC "Tổng nhận" hiển thị trên bảng chính `/luong` (chưa có khái niệm phụ cấp cơm trưa) — không phải sai lệch, xem giải thích đầu file. Ngày chấm công trong lưới lấy TỪ `Attendance` (chỉ có/không — schema hiện KHÔNG hỗ trợ nửa ngày).
...
- **lookups.ts** — `getActiveServices/getActiveMaterials/getConsultants/getDoctors/getNurses`. `getConsultants` trả **MỌI nhân sự đang hoạt động** (ai cũng có thể là người tư vấn).
...
- **export.ts** + **xlsx.ts** — xuất CSV/Word(.doc)/Excel(.xlsx THẬT, tự dựng ZIP, không thư viện ngoài). `buildXlsx`/`xlsxResponse` = bảng cột-hàng đơn giản (đa số route export dùng). `buildRichXlsx`/`xlsxRichResponse` = layout tự do theo toạ độ ô (merge, viền, font, canh giữa, khổ ngang) cho các mẫu biểu công ty cần đúng hình thức — vd bảng lương kế toán (`lib/payroll-letterhead.ts`), phụ lục hoa hồng (`lib/commission-appendix.ts`).
- **ai.ts** — gọi AI và planner, **TRUNG LẬP NHÀ CUNG CẤP** (không khoá vào một hãng). `resolveAiConfig(env)` THUẦN (có test) phân giải `AI_API_KEY`/`AI_BASE_URL`/`AI_MODEL`/`AI_PROVIDER`; `generateStructured` hỗ trợ thêm `AI_AGENT_MODEL` để planner nhiều bước dùng model riêng mạnh hơn mà không đổi model soạn tin. Hỗ trợ OpenAI-compatible (`/chat/completions` — DeepSeek, Qwen, Gemini-compat, OpenAI, Groq, tự host Ollama/vLLM) và Anthropic (`/v1/messages` — Claude). Tương thích ngược `ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL`. Ẩn nút AI nếu chưa có khoá (`aiConfigured()`). KHÔNG gửi dữ liệu nhạy cảm không cần thiết. Cấu hình xem `.env.example`.
...
- **assistant.ts** — trợ lý AI hỏi-đáp và knowledge map (có test, D1): `formatAssistantContext` (ảnh chụp số liệu → văn bản gọn), `BUSINESS_RULES_KNOWLEDGE` (giải thích cơ chế nghiệp vụ THẬT — giá dịch vụ, lương, công nợ, hạng thành viên, Lãi/Lỗ — viết tay bằng ngôn ngữ kinh doanh, KHÔNG thuật ngữ code; sửa công thức ở lib nào thì cập nhật đoạn tương ứng ở đây, đối chiếu mục 7 bên dưới), `ASSISTANT_SYSTEM`, `SUGGESTED_QUESTIONS`. **assistant-data.ts**: `getAssistantContext()` gom số liệu KINH DOANH (KHÔNG gồm SĐT/y khoa) — tái dùng `getBusinessAnalytics`. Dùng ở `/tro-ly` (module `tro-ly`, icon `Sparkles`, **CHỈ ADMIN + SHAREHOLDER** — ranh giới cứng ở `userCan()`, không có MANAGER dù trang gần giống `/ke-hoach`); action `askAssistant` đưa CẢ bối cảnh số liệu LẪN `BUSINESS_RULES_KNOWLEDGE` cho `ai.ts` — AI KHÔNG truy cập thẳng DB (chống bịa số/rò rỉ). 🔑 cần `AI_API_KEY`.
  - **assistant-request-ai.ts** — vẫn giữ luồng "Ghi thành yêu cầu cho lập trình" để lưu một đề xuất vào `/ke-hoach`. Với thao tác nghiệp vụ đã có tool, Gateway r6 không còn chỉ ghi yêu cầu: `tro-ly/agent.ts` có registry đọc/ghi hồ sơ, sổ tư vấn, chấm công, lương và chứng từ, luôn kiểm quyền server-side + preview + approval + audit. Với yêu cầu đổi code/cơ chế, `propose_system_change` tạo PlanTask cha kèm checklist 5 bước; không tự sửa code production và không bỏ qua diff/test/backup/triển khai có kiểm soát.
...
**Enums**: `Role` (ADMIN, MANAGER, TELESALE, RECEPTION, CONSULTANT, DOCTOR, NURSE, CARE, SHAREHOLDER, **COLLABORATOR**), `EmploymentStatus` (ACTIVE, RETIRED), `Gender`, `CustomerSource`, `AppointmentType`, `AppointmentStatus`, `CaseStatus` (OPEN, CONSULTED, SERVICED, COMPLETED, CANCELLED), `ConsultResult` (PENDING, AGREED, CONSIDERING, DECLINED), `PaymentMethod` (CASH, CARD, TRANSFER, EWALLET), `PhotoType` (BEFORE, AFTER, FOLLOW_UP, **CLINICAL**), `CareChannel`, `CareDirection`, `StockType`, `CashType` (INCOME, EXPENSE), `PlanTaskStatus` (TODO, IN_PROGRESS, DONE), `ChannelKind` (ZALO_OA, FACEBOOK), `MessageStatus` (RECEIVED, SENT, FAILED).

**Models chính**:
- **User** — nhân sự + thông tin HR đầy đủ (dob, gender, nationalId, hometown, address, bank*, emergency*, position, department, hireDate, qualification, notes), `baseSalary`, `avatarUrl`, `role`, `permissions` (JSON `{grant,deny}`), `totpSecret/totpEnabled`, `employmentStatus`/`retiredAt`. **KHÔNG còn `commissionRate`** (đã bỏ — hoa hồng nhập tay). Nhân sự nghỉ việc giữ nguyên userId/lịch sử nhưng `active=false`.
- **Customer** — `phoneEnc`(mã hoá)/`phoneLast5`, `source`/`sourceDetail` (snapshot), `collaboratorId`/`collaboratorAssignedAt`, `portalToken`, `code`. An toàn y khoa (B6 gđ1): `allergies`/`medicalHistory`/`contraindications` (text, hiện banner cảnh báo ở trang khách + hồ sơ qua `components/ui/medical-alert.tsx`).
...
- **CaseService** (`listPrice`=giá gốc, `unitPrice`=giá ưu đãi, `quantity`, `discount`, `finalPrice`, `doctorId`=bác sĩ trực tiếp thực hiện, `nurseId`=điều dưỡng phụ trách — cả 2 optional, dùng tính hoa hồng theo dịch vụ ở `lib/commission-data.ts`), **Payment** (`amount`, `method`, `paidAt`, `receivedById`), **MaterialUsage**, **Photo** (`type`, `url`, `caption`, `followUpIndex`, `caseId` nullable — giữ ảnh khi xóa hồ sơ).
...
- **ChannelAccount** + **Conversation** + **Message** (Kênh giao tiếp — Zalo OA/Facebook, xem mục "Kênh giao tiếp (Omnichannel)" bên dưới mục 7). KHÁC **CareMessage** (nhật ký chăm sóc: `channel`, `direction`), **FollowUp** (hẹn tái khám: `status`/`doneAt` được CẬP NHẬT THẬT qua nút "Đã đến" — `markFollowUpArrived` ở `ho-so/actions.ts` — hiện ở thẻ Tái khám của hồ sơ, thẻ khách hàng, mục "Tái khám đến hạn" của `/viec-hom-nay`, và **từ Đợt 33 hiện GỘP CHUNG luôn với `Appointment` trên `/lich-hen`** (xem cạm bẫy #18) — trước đây tách bảng riêng nên trang Lịch hẹn hoàn toàn không biết tới lịch tái khám đặt từ hồ sơ; trước đây 2 trường `status`/`doneAt` chỉ có giá trị mặc định lúc tạo, không nơi nào cập nhật được dù `workqueue.ts` đã lọc theo chúng), **AuditLog**, **CashTransaction** (sổ thu chi), **Collaborator** (hồ sơ CTV: `userId`, `name @unique`, `phone`, `bank*`, `note`, `active`, liên kết Customer/Case/Payout). Cổng CTV nằm ở `/cong-tac-vien-cua-toi`, chỉ đọc dữ liệu thuộc CTV trong 6 tháng và che SĐT còn 5 số cuối.
...
- **COLLABORATOR (Cộng tác viên)**: role bị giới hạn cứng ở `mod:cong-tac-vien-cua-toi` và `mod:khach-hang`; không nhận grant mở rộng, không có `phone.full`, không có quyền ghi. Query Customer/Case phải lọc theo `collaboratorId` và `collaboratorAssignedAt >= now - 6 tháng`.
...
- **Hoa hồng tự động gợi ý** (xem `lib/commission.ts`/`commission-data.ts` ở đầu mục 4): mỗi dòng bảng lương hiện thêm dòng nhỏ "gợi ý X" dưới cột Hoa hồng khi số hệ thống tính khác số đã lưu; modal sửa lương (`PayrollEditButton`) có khối "Hệ thống tính được ..." kèm nút "Dùng số này" điền vào ô Hoa hồng — vẫn CHỈ LÀ GỢI Ý, KHÔNG tự lưu (cùng triết lý "mang số tháng trước làm mốc" ở trên).
- **Xuất bảng lương kế toán** (`/luong/export-ke-toan?m=&d=`, quyền `mod:luong` — khớp đúng quyền trang `/luong`, xem `lib/payroll-letterhead.ts` + `lib/commission-appendix.ts`): 1 file .xlsx đúng mẫu công ty chủ cung cấp (tiêu đề công ty/quốc hiệu, lưới ngày công, Công thực tế/Phụ cấp cơm trưa/Hoa Hồng Tư Vấn/Tiền Thủ Thuật/Tổng cộng hưởng lương, chữ ký 3 bên) **+ 1 sheet phụ lục hoa hồng riêng cho MỖI nhân sự có phát sinh trong tháng** — "1 cái click" đủ dữ liệu cho kế toán/thuế, không cần mở nhiều file. Nút "Bảng lương kế toán (mẫu chuẩn + phụ lục hoa hồng)" nằm trong dropdown `ExportMenu` ở `/luong` (component nhận thêm prop `extra` cho các mục xuất file tuỳ biến ngoài Excel/Word/CSV chuẩn). Xây bằng `buildRichXlsx` (`lib/xlsx.ts`, hỗ trợ merge ô/viền/font/canh giữa/khổ ngang — tách khỏi `buildXlsx` gốc để không ảnh hưởng các export đơn giản khác).
...
- **Khoá mã hoá**: `PHONE_ENC_KEY` phải được cấp từ secret manager/.env hoặc volume runtime; không có fallback cố định trong source. Nếu mất khóa khi CSDL còn dữ liệu mã hóa, entrypoint dừng khởi động.
- **Mật khẩu demo**: seed chỉ dành cho QA cô lập và bắt buộc nhận `DEMO_PASSWORD` từ môi trường (tối thiểu 12 ký tự); không có mật khẩu mặc định.
- **Toán tiền nguyên tử** (A3): mọi thao tác động tới tiền trong hồ sơ chạy qua `withCaseLock` (`$transaction` + `FOR UPDATE`). Toán đặt ở `lib/financial-summary.ts` (`summarizeCase`/`correctedFinalPrice`, thuần, có test) — sửa logic tiền thì sửa ở đó + cập nhật test. (File `case-math.ts` cũ từng định làm việc này nhưng không còn nơi nào gọi tới — đã xóa 17/8/2026, tránh sửa nhầm chỗ không có tác dụng.)
- **`AUTH_SECRET`**: tự sinh ngẫu nhiên mỗi máy, lưu trong volume `zenith_secrets` (KHÔNG commit). Có thể đặt riêng qua `.env`.
- **`PHONE_ENC_KEY`**: phải backup secret cùng CSDL; đổi khoá cần **mã hoá lại** dữ liệu (`prisma/rotate-phone-key.ts`) — KHÔNG tự đổi.
- **Khi đóng gói code gửi ra ngoài**: loại trừ `.env`/secrets và không đưa `DEMO_PASSWORD` vào source.
...
- Repo clone vào **`%USERPROFILE%\ZenithTasks`**. Script ở **`%USERPROFILE%\ZenithTasks\windows\`**:
  - ⭐ **`Chay-Zenith.bat`** — **NÚT CHÍNH chủ hay bấm** (cài / cập nhật / chạy). Script chỉ kéo `master` bằng `pull --ff-only` khi repo sạch, đảm bảo service `db` đang chạy, build riêng `app`, rồi `docker compose up -d --no-deps --force-recreate app`; migration tự chạy trong entrypoint. Health-check kỹ thuật dùng `http://127.0.0.1:3000`, không dùng `localhost` vì Windows có thể ưu tiên `[::1]`.
  - **`Sua-Loi.bat`** — dùng khi cập nhật lỡ dở/đổi schema: bảo vệ thay đổi local bằng backup branch + stash có kiểm soát, build `app --no-cache`, recreate riêng `app`, migrate và kiểm tra `http://127.0.0.1:3000/login`. Nếu recreate app lỗi, script không khẳng định bản cũ vẫn phục vụ; database/volume không bị reset.
  - **`Mo-App.bat`** (mở), **`Phat-Hanh-Mang`/`Dia-Chi-Co-Dinh`** (Cloudflare Tunnel ra Internet; origin luôn `http://127.0.0.1:3000`, không đổi hostname public), **`Kiem-Tra-Phat-Hanh.bat`** (kiểm tra đọc-only, tạo evidence trước/sau release), **`Sao-Luu`/`Cai-Sao-Luu-Tu-Dong`** (sao lưu DB + uploads), **`Cai-Tu-Dong-Cap-Nhat`/`Tu-Dong-Cap-Nhat`** (tự động cập nhật hằng ngày — xem bên dưới), **`Cai-AI-Key`** (trình chọn nhà cung cấp AI — DeepSeek/Qwen/Gemini/OpenAI/Claude/tự host — rồi ghi `AI_API_KEY`/`AI_BASE_URL`/`AI_MODEL`/`AI_PROVIDER` vào `.env`), **`Doi-Khoa-SDT`** (tự động đổi `PHONE_ENC_KEY`: sao lưu → tạo khoá → mã hoá lại Customer+Lead → cập nhật `.env` → restart; tự dừng + giữ khoá cũ nếu lỗi), **`Xem-Loi`** (xuất log).
  - **`Cai-Tu-Dong-Cap-Nhat.bat`** (chạy 1 lần) — hẹn lịch Windows Task Scheduler (`ZenithTuDongCapNhat`) chạy **`Tu-Dong-Cap-Nhat.ps1`** mỗi ngày lúc 02:00 sáng: chỉ cập nhật khi repo sạch; dùng `pull --ff-only`, build riêng `app`, `up -d --no-deps --force-recreate app`, migrate và health-check `http://127.0.0.1:3000/login`. Nếu repo bẩn/build/recreate/migration lỗi, script dừng và ghi log thay vì stash/reset hoặc tuyên bố sai. Ghi nhật ký vào `%USERPROFILE%\zenith-tu-dong-cap-nhat.log`. Tắt bằng Task Scheduler (Windows) → tìm `ZenithTuDongCapNhat` → Disable/Delete.
...
- Đăng nhập lần đầu: production dùng tài khoản bootstrap cá nhân; QA seed dùng `DEMO_PASSWORD` cấp từ môi trường.
...
- ⚠️ **17/8/2026: đã gộp nhánh `claude/lucid-cori-fg136w` (86 commit riêng — tự động hoa hồng, Trợ lý AI, vá bảo mật/tài chính) vào `master` (20 commit riêng — hộp thư Zalo/Facebook) và xoá `claude/lucid-cori-fg136w`.** `master` giờ là nhánh DUY NHẤT. Các script `windows/*.bat|ps1` đã cập nhật để kéo code từ `master`. Nếu thấy tài liệu/code nào còn nhắc `claude/lucid-cori-fg136w` là do sót lại, sửa về `master`.
...
18. **Đặt lịch tái khám cho khách cũ (từ hồ sơ điều trị) không hiện ở `/lich-hen`**: trang `/lich-hen` chỉ đọc bảng `Appointment`, trong khi nút "Hẹn tái khám" trong hồ sơ (`addFollowUp`) ghi vào bảng **`FollowUp`** — 2 model tách biệt hoàn toàn từ đầu (khác với `Appointment.type` có sẵn giá trị `FOLLOW_UP` dễ gây nhầm là cùng nguồn dữ liệu). Dữ liệu vẫn lưu đúng và hiện đủ ở thẻ Tái khám của hồ sơ/khách hàng + "Việc cần làm hôm nay", chỉ riêng `/lich-hen` là "mù" với tái khám vì chưa từng query bảng này. **Không gộp 2 model làm một** được vì `Appointment.caseId` là `@unique` (1 lịch hẹn ứng 1 hồ sơ), trong khi 1 hồ sơ có thể có NHIỀU lần tái khám theo thời gian. Cách sửa: `lich-hen/page.tsx` giờ truy vấn CẢ HAI, gộp thành 1 danh sách `rows` (`{kind:"appointment"|"followup", ...}`) sắp theo `scheduledAt`, dùng chung cho khung ngày/tháng/thẻ tóm tắt/đếm "đã đến" — tái khám hiện badge "Tái khám" (tím, cùng tông với `APPT_TYPE.FOLLOW_UP`), dùng lại `FollowUpArriveButton`/`deleteFollowUp` đã có sẵn (không viết action mới). Bài học: khi 1 nghiệp vụ có VẺ giống nhau nhưng tách bảng vì lý do quan hệ dữ liệu khác nhau (ở đây: 1-1 vs 1-nhiều với hồ sơ), các trang tổng hợp theo NGÀY/THÁNG phải chủ động gộp cả hai nguồn — không thể giả định "đã có bảng chung".
19. **`developers.zalo.me` chặn truy cập tự động (403) với công cụ fetch của AI** — không lấy được nguyên văn tài liệu chính thức về công thức ký webhook (`mac`)/endpoint OAuth khi xây `lib/channels/zalo.ts`. Đã tổng hợp từ NHIỀU nguồn cộng đồng độc lập khớp nhau (không phải đoán mò) nhưng CHƯA kiểm chứng với traffic thật. Nếu sau này Zalo OA thật kết nối mà webhook báo "chữ ký không hợp lệ" hàng loạt (log `console.warn` ở `api/webhooks/zalo/route.ts`), đây là chỗ đầu tiên cần rà — thử fetch tài liệu qua trình duyệt thật (không phải tool tự động) hoặc hỏi trực tiếp support Zalo OA.

## 13.5. Bổ sung 2026-08-18 — chứng từ, sổ tư vấn, HR và AI

### Lương và hoa hồng theo thực thu
Hoa hồng tự động lấy tiền khách đã thanh toán theo từng Payment trong kỳ, không lấy giá trị chốt và không lấy phần còn nợ. Khách trả góp đến đâu thì kỳ đó tính đến đó. Nếu có phân bổ phối hợp, hệ thống chia theo tỷ lệ đã lưu; phần điều chỉnh thủ công phải nhập vào `commissionOverride`, không nhập lại hoa hồng tự động.

### Giấy đề nghị thanh toán
Mở menu **Đề nghị thanh toán** hoặc truy cập `/ke-toan/de-nghi-thanh-toan`. Mỗi khoản chi có thể tạo thành phiếu, sau đó ADMIN duyệt hoặc từ chối, in bản Word/HTML và ghi sổ PAID. Chi lương và chi hoa hồng cộng tác viên tạo chứng từ liên kết tự động. Không dùng chứng từ này để thay thế hóa đơn, chứng từ ngân hàng hoặc quy định kế toán/thuế cần thiết; kế toán phải rà soát bộ hồ sơ thực tế.

### Sổ tư vấn điện tử
Trong hồ sơ điều trị, nhập sổ tư vấn gồm hành chính bổ sung, sinh hiệu, 18 câu sàng lọc, mong muốn, hiện trạng, kết quả dự tính, chỉ định và xác nhận khách. Nút đánh dấu nhanh chỉ giúp nhập nhanh thông tin khai báo, không thay thế đánh giá chuyên môn. Bản ghi được chỉnh sửa trong 24 giờ; sau thời hạn chỉ ADMIN được sửa và hệ thống ghi audit. Dùng nút **In sổ tư vấn** để xuất bản Word/HTML.

### Thỏa thuận nhân sự
Tại `/nhan-su/[id]/thoa-thuan`, ADMIN tạo bản nháp bảo mật hoặc không cạnh tranh, theo dõi version/snapshot, thời hạn, đánh dấu đã ký hoặc thu hồi và in snapshot. Các điều khoản pháp lý, thời hạn, phạm vi và mức phạt cần luật sư/lao động rà soát trước khi ký chính thức.

### AI upload, feedback và giọng nói
Trong `/tro-ly`, nút kẹp giấy cho phép tải TXT, CSV, JSON, Word, Excel, PDF hoặc ảnh tối đa 15MB. Nội dung văn bản được trích xuất server-side và giữ tối đa 30 ngày; file không trích xuất được chữ vẫn được lưu metadata nhưng AI không được tự đoán nội dung. Nút micro dùng Web Speech API để điền lời nói vào ô nhập; anh vẫn phải bấm gửi. Nút **Đúng/Cần sửa** lưu feedback có audit theo tài khoản, dùng làm ngữ cảnh tham khảo cho các lượt sau chứ không phải tự huấn luyện lại mô hình.
