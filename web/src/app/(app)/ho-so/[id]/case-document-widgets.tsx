 ...
 */
export function UploadDocumentButton({ caseId }: { caseId: string }) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Tải giấy tờ hành chính">
        <form action={action} className="space-y-4">
          <input type="hidden" name="caseId" value={caseId} />
          <div>
            <Label htmlFor="doc-title">Tên giấy tờ *</Label>
            <Input id="doc-title" name="title" placeholder="VD: Phiếu đồng ý phẫu thuật, CMND/CCCD…" required autoFocus />
          </div>
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-400">Hỗ trợ PDF, ảnh (JPG/PNG), Word, Excel — tối đa 15MB. Tải file đã soạn/đã ký sẵn lên.</p>
          </div>
